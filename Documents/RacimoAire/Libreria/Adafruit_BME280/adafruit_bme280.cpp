...
*/
/**************************************************************************/
Adafruit_BME280::Adafruit_BME280(Crpigpio *cGPIO, uint8_t i2cAddress = BME280_ADDRESS)
{
    if(!begin(cGPIO, i2cAddress))
    {
       m_InitOk = -1;
       qDebug() << "Error al abrir dispositivo \"BME280\"";
    }
    else
        m_InitOk = 1;
}
...
bool Adafruit_BME280::begin(Crpigpio *cGPIO,uint8_t i2cAddress)
{
    _i2caddr = i2cAddress;
    RA_cGPIO = cGPIO;
    return init();
...
*/
/**************************************************************************/
void Adafruit_BME280::write8(unsigned reg, uint8_t value) {
    RA_cGPIO->WriteDeviceI2C(reg,value);
}
...
*/
/**************************************************************************/
uint8_t Adafruit_BME280::read8(unsigned reg) {
    uint8_t ReturnData;
    char buf[1];
    RA_cGPIO->ReadDeviceI2C(reg, &buf[0], 1);
    ReturnData = buf[0] ;
     return ReturnData;
}
...
*/
/**************************************************************************/
uint16_t Adafruit_BME280::read16(unsigned reg)
{
    uint16_t ReturnData;
    char buf[2];
...
*/
/**************************************************************************/
uint16_t Adafruit_BME280::read16_LE(unsigned reg) {
...
*/
/**************************************************************************/
int16_t Adafruit_BME280::readS16(unsigned reg)
{
...
*/
/**************************************************************************/
int16_t Adafruit_BME280::readS16_LE(unsigned reg)
{
...
*/
/**************************************************************************/
uint32_t Adafruit_BME280::read24(unsigned reg)
{
    uint32_t ReturnData;
    char buf[3];
    RA_cGPIO->ReadDeviceI2C(reg, &buf[0], 3);
    ReturnData = ( buf[0] << 16 ) | (buf[1] << 8) | buf[2];
    return ReturnData;
}
...
int Adafruit_BME280::InitOk() const
{
    return m_InitOk;
}
