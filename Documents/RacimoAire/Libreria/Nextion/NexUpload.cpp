...
bool NexUpload::_searchBaudrate(uint32_t baudrate)
{
    ...
    this->sendCommand(Serial, Serial, "");
    this->sendCommand(Serial, Serial, "connect");
    this->recvRetString(Serial,string);
...
void NexUpload::sendCommand(Serial, Serial, const char* cmd)
{
...
uint16_t NexUpload::recvRetString(Serial,String &string, uint32_t timeout,bool recv_flag)
{
    uint16_t ret = 0;
    uint8_t c = 0;
    long start;
...
bool NexUpload::_setDownloadBaudrate(uint32_t baudrate)
{
    ...
    dbSerialPrintln(cmd);
    this->sendCommand(Serial, Serial, "");
    this->sendCommand(Serial, Serial, cmd.c_str());
    delay(50);
    nexSerial.begin(baudrate);
    this->recvRetString(Serial,string,500);
    if(string.indexOf(0x05) != -1)
    { 
        return 1;
    } 
...
bool NexUpload::_downloadTftFile(void)
{
    ...
    while(send_timer)
    {
        ...
        this->recvRetString(Serial,string,500,true);
        if(string.indexOf(0x05) != -1)
        { 
            string = "";
        } 
