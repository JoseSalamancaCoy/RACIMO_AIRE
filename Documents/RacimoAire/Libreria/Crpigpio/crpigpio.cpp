#include "crpigpio.h"
#include <QDebug>
Crpigpio::Crpigpio(bool Init,QObject *parent): QObject(parent)
{
    if(Init)
    {
        if(gpioInitialise()<0)
...
int8_t Crpigpio::InvertirBits(int8_t entero)
{
    int8_t enteroInvertido=0;
    while(entero>0)
    {
        enteroInvertido = (enteroInvertido << 1) + (entero & 1);
        entero = entero >> 1;
    }
    return enteroInvertido;
}

uint8_t Crpigpio::InvertirBits(uint8_t entero)
{
    uint8_t enteroInvertido=0;
    while(entero>0)
    {
        enteroInvertido = (enteroInvertido << 1) + (entero & 1);
        entero = entero >> 1;
...
int16_t Crpigpio::InvertirBits(int16_t entero)
{
    int16_t enteroInvertido=0;
    while(entero>0)
    {
        enteroInvertido = (enteroInvertido << 1) + (entero & 1);
        entero = entero >> 1;
...
uint16_t Crpigpio::InvertirBits(uint16_t entero)
{
    uint16_t enteroInvertido=0;
    while(entero>0)
    {
        enteroInvertido = (enteroInvertido << 1) + (entero & 1);
        entero = entero >> 1;
...
/*
int32_t Crpigpio::InvertirBits(int32_t entero)
{
    int32_t enteroInvertido=0;
    while(entero>0)
    {
        enteroInvertido = (enteroInvertido << 1) + (entero & 1);
        entero = entero >> 1;
...
uint32_t Crpigpio::InvertirBits(uint32_t entero)
{
    uint32_t enteroInvertido=0;
    while(entero>0)
    {
        enteroInvertido = (enteroInvertido << 1) + (entero & 1);
        entero = entero >> 1;
...
int Crpigpio::InvertirBits(int entero)
{
    int enteroInvertido=0;
    while(entero>0)
    {
        enteroInvertido = (enteroInvertido << 1) + (entero & 1);
        entero = entero >> 1;
...
char Crpigpio::InvertirBits(char entero)
{
    char enteroInvertido;
    enteroInvertido = enteroInvertido<<8;
    while(entero>0)
    {
        enteroInvertido = (enteroInvertido << 1) + (entero & 1);
        entero = entero >> 1;
...
//Liberar SPI
int Crpigpio::CloseSPI()
{
    int ret= spiClose(HanleSPI);
    if(ret<0 )qDebug() << GetErrorCode(ret);
    return ret;
}

//Leer bytes con el protocolo SPI
...
//Escribir bytes con el protocolo SPI
int Crpigpio::WriteSPI(QByteArray &arr)
{
 ...
 //Escribir y leer bytes con el protocolo SPI
...
//Leer del buffer del GPIO la informacion serial
...
int Crpigpio::OpenSerial(unsigned baud, unsigned serFlag)
{
    char AddrSerial[]="/dev/ttyS0";
    // char AddrSerial[]="/dev/serial1";
    //char AddrSerial[]="/dev/ttyACM0";
    qDebug() << "abriendo serial";
    HandleSerial = serOpen(&AddrSerial[0],baud, serFlag )   ;
    qDebug() << HandleSerial;
    if(HandleSerial<0 )qDebug() << GetErrorCode(HandleSerial);
    return HandleSerial;
}

//Liberar puerto Serial
int Crpigpio::CloseSerial()
{
    int ret = serClose(HandleSerial )   ;
    if(ret<0 )qDebug() << GetErrorCode(ret);
    return ret;
}

//Leer un byte Serial
int Crpigpio::ReadByteSerial(bool Show)
{
    int ret = serReadByte(HandleSerial);
    if(ret<0 && Show)qDebug() << GetErrorCode(ret);
    return ret;
}

int Crpigpio::WriteByteSerial(unsigned Val)
{
    int ret = serWriteByte(HandleSerial,Val) ;
    if(ret<0 )qDebug() << GetErrorCode(ret);
    return ret;
}

int Crpigpio::WriteByteSerial(char Val)
{
    unsigned Valunsigned =Val;
    int ret = serWriteByte(HandleSerial,Valunsigned) ;
    if(ret<0 )qDebug() << GetErrorCode(ret);
    return ret;
}

//Escribir un byte Serial
int Crpigpio::WriteSerial(char *buf, unsigned count)
{
    int ret = serWrite(HandleSerial ,buf, count )   ;
    if(ret<0 )qDebug() << GetErrorCode(ret);
    return ret;
}

//Leer bytes Seriales
int Crpigpio::ReadSerial(char *buf, unsigned count)
{
    int ret = serRead(HandleSerial ,buf, count )   ;
    if(ret<0 )qDebug() << GetErrorCode(ret);
    return ret;
}

//Determinar numero de bytes disponibles en el puerto
int Crpigpio::AvailableSerial()
{
    int ret = serDataAvailable(HandleSerial )   ;
    if(ret<0 )qDebug() << GetErrorCode(ret);
    else if(ret !=0) emit OnCallSerial(ret);
    return ret;
}
...
int Crpigpio::OpenI2C(unsigned Bus_i2c, unsigned Addr_i2c, unsigned Flags_i2c)
{
    Handlei2c =i2cOpen(Bus_i2c,Addr_i2c,Flags_i2c);
    if(Handlei2c>0)return Handlei2c;
    if(Handlei2c<0 )qDebug() << GetErrorCode(Handlei2c);
    return Handlei2c;
}


int Crpigpio::WriteAndReadDeviceI2C(unsigned i2cReg,char *bufi2c, unsigned counti2cWrite)
{
    int i2c_ret = i2cBlockProcessCall(Handlei2c, i2cReg,bufi2c, counti2cWrite);
    if(i2c_ret<0)qDebug() << GetErrorCode(i2c_ret);
    return i2c_ret;

}
int Crpigpio::ReadDeviceI2C(char *bufi2c, unsigned counti2c)
{
    int i2c_ret = i2cReadDevice(Handlei2c,bufi2c,counti2c);
    if(i2c_ret<0)qDebug() << GetErrorCode(i2c_ret);
    return i2c_ret;
...
int Crpigpio::ReadDeviceI2C(unsigned i2cReg, uint8_t *returnData)
{
    int i2c_ret = i2cReadByteData(Handlei2c,i2cReg);
    if(i2c_ret<0)qDebug() << GetErrorCode(i2c_ret);
    *returnData = static_cast<uint8_t>(i2c_ret);
    return i2c_ret;
}

int Crpigpio::ReadDeviceI2C(unsigned i2cReg, uint16_t *returnData)
{
    int i2c_ret = i2cReadWordData(Handlei2c,i2cReg);
    if(i2c_ret<0)qDebug() << GetErrorCode(i2c_ret);
    *returnData = static_cast<uint16_t>(i2c_ret);
    return i2c_ret;
}

int Crpigpio::ReadDeviceI2C(unsigned i2cReg, int16_t *returnData)
{
    int i2c_ret = i2cReadWordData(Handlei2c,i2cReg);
    if(i2c_ret<0)qDebug() << GetErrorCode(i2c_ret);
    qDebug() << QString("Lectura I2C %1 .... %2").arg(i2c_ret).arg(i2c_ret,0,2);
    int enteroInvertido=0;
    int entero =i2c_ret;
    for(int i=0; i < 16; i++)
    {
    ...
    qDebug() << QString("Lectura Invertida I2C %1 .... %2").arg(enteroInvertido).arg(enteroInvertido,0,2);
    *returnData = static_cast<int16_t>(i2c_ret);
    return i2c_ret;
}

int Crpigpio::ReadDeviceI2C(unsigned i2cReg, char *bufi2cRead,unsigned count)
{
    int i2c_ret = i2cReadI2CBlockData(Handlei2c,i2cReg,bufi2cRead,count);
    if(i2c_ret<0)qDebug() << GetErrorCode(i2c_ret);
    return i2c_ret;
}

int Crpigpio::WriteDeviceI2C(char *bufi2c, unsigned counti2c)
{
    int i2c_ret = i2cWriteDevice(Handlei2c,bufi2c,counti2c);
    if(i2c_ret<0)qDebug() << GetErrorCode(i2c_ret);
    return i2c_ret;
}

int Crpigpio::WriteDeviceI2C(unsigned i2cReg, uint8_t Wval)
{
    int i2c_ret = i2cWriteByteData(Handlei2c,i2cReg,Wval);
    if(i2c_ret<0)qDebug() << GetErrorCode(i2c_ret);
    return i2c_ret;
}

int Crpigpio::WriteDeviceI2C(unsigned i2cReg, uint16_t Wval)
{
    int i2c_ret = i2cWriteWordData(Handlei2c,i2cReg,Wval);
    if(i2c_ret<0)qDebug() << GetErrorCode(i2c_ret);
    return i2c_ret;
}
...
int Crpigpio::CloseI2C()
{
    int ret = i2cClose(Handlei2c);
    if(ret<0)qDebug() << GetErrorCode(ret);
    return ret;
}

//************************* Establecer GPIO como PWM ***************************
...
//Cambiar frecuencia PWM del GPIO
...
//Configurar como entrada o salida el GPIO
...
//Establecer el GPIO con pull up o pull down
...
//************************* Interrupciones *****************************************
//Establecer como GPIO de interrupcion
...
//************************** Obtener error de funcion PIGPIO *********************
